                        ...
                        @foreach($messages as $messageGroup)
                            <div class="media {{$messageGroup->first()->shouldAddUnreadClass()?'unread':''}}">
                                <div class="media-left">
                                    <a href="#">
                                        @if(\Illuminate\Support\Facades\Auth::id()==$messageGroup->last()->from_user_id)
                                            <img width="48px" src="{{$messageGroup->last()->toUser->avatar}}" alt="">
                                        @else
                                            <img width="48px" src="{{$messageGroup->last()->fromUser->avatar}}" alt="">
                                        @endif
                                    </a>
                                </div>
